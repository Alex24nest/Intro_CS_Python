...
def text_to_list(input_text):
    ...
    for c in input_text:
        if c in 'zxcvbnmasdfghjklqwertyuiop':
            word += c
        elif word:
            n_list.append(word)
            word = ''
...
def get_letter_frequencies(word):
    ...
    """
    return get_frequencies([i for i in word])
...
def get_tf(file_path):
    ...
    * Think about how we can use get_frequencies from earlier
    ...
    words_list = text_to_list(file)

    dict = get_frequencies(words_list)
    ALL = sum(dict.values())
    for k,v in dict.items():
        dict[k] = v/ALL
    return dict
...
def get_idf(file_paths):
    ...
    * IDF is calculated as IDF(i) = log_10(total number of documents / number of
    ...
    for k in dict.keys():
        for p in file_paths:
            if k in text_to_list(load_file(p)):
                dict[k] += 1

    for k,v in dict.items():
        dict[k] = math.log10(len(file_paths)/v)
...
def get_tfidf(tf_file_path, idf_file_paths):
    """
        Args:
            tf_file_path: name of file in the form of a string (used to calculate TF) idf_file_paths: list of names of files, where each file name is a string (used to calculate IDF)
        Returns:
           a sorted list of tuples (in increasing TF-IDF score), where each tuple is of the form (word, TF-IDF). In case of words with the same TF-IDF, the words should be sorted in increasing alphabetical order.
    ...
    for k,v in TF.items():
        for l,b in IDF.items():
            if k == l:
                L_retr.append((k, v*b))

    return sorted(L_retr, key=lambda x: (x[1], x[0]))
